...
#endif
// TODO add enum for type and reason

// MARK: - Model

open class NotificationThreadSubject: Codable {
...
open class NotificationThread: Codable {
    ...
    open private(set) var subject = NotificationThreadSubject()
...
enum NotificationRouter: Router {
    ...
    var method: HTTPMethod {
        switch self {
            case .readNotifications(_, _, _, _, _),
                 .getNotificationThread(_, _),
                 .getThreadSubscription(_, _),
                 .listRepositoryNotifications(_, _, _, _, _, _, _, _, _):
                return .GET
            case .markNotificationsRead(_, _, _),
                 .setThreadSubscription(_, _, _),
                 .markRepositoryNotificationsRead(_, _, _, _):
                return .PUT
            case .markNotificationThreadAsRead(_, _):
                return .POST
            case .deleteThreadSubscription(_, _):
                return .DELETE
        }
    }
    ...
    var params: [String: Any] {
        switch self {
            ...
            case .getNotificationThread(_, _):
                return [:]
            case .markNotificationThreadAsRead(_, let threadID):
                return ["thread_id": threadID]
            case .getThreadSubscription(_, _):
                return [:]
            case .setThreadSubscription(_, _, _):
                return [:]
            case .deleteThreadSubscription(_, _):
                return [:]
