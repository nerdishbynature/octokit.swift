...
class LabelTests: XCTestCase {
    ...
    func testGetLabel() {
        let session = OctoKitURLTestSession(
            expectedURL: "https://api.github.com/repos/octocat/hello-world/labels/bug",
    ...
    func testGetLabelEncodesSpaceCorrectly() {
        let session = OctoKitURLTestSession(
            expectedURL: "https://api.github.com/repos/octocat/hello-world/labels/help%20wanted",
            expectedHTTPMethod: "GET",
            jsonFile: nil,
            statusCode: 200
        )
        let task = Octokit()
            .label(session, owner: "octocat", repository: "hello-world", name: "help wanted") { response in
                switch response {
                case .success:
                    XCTAssert(true)
                case .failure:
    ...
    func testGetLabelsSetsPagination() {
        let session = OctoKitURLTestSession(
            expectedURL: "https://api.github.com/repos/octocat/hello-world/labels?page=2&per_page=50",
            expectedHTTPMethod: "GET",
            jsonFile: nil,
            statusCode: 200
        )
        let task = Octokit()
            .labels(session, owner: "octocat", repository: "hello-world", page: "2", perPage: "50") { response in
                switch response {
                case .success:
                    XCTAssert(true)
                case .failure:
    ...
    func testCreateLabel() {
        let session = OctoKitURLTestSession(
            expectedURL: "https://api.github.com/repos/octocat/hello-world/labels",
            expectedHTTPMethod: "POST",
            jsonFile: "label",
            statusCode: 200
        )
        let task = Octokit()
            .postLabel(session, owner: "octocat", repository: "hello-world", name: "test label",
                       color: "ffffff") { response in
                switch response {
                case let .success(label):
