...
class ConfigurationTests: XCTestCase {
    ...
    func testTokenConfigurationWithPreviewHeaders() throws {
        let subject = TokenConfiguration("12345", previewHeaders: [.reactions])
        XCTAssertEqual(subject.accessToken, "12345".data(using: .utf8)?.base64EncodedString())
        XCTAssertEqual(subject.apiEndpoint, "https://api.github.com")
        XCTAssertNotNil(subject.customHeaders)
        XCTAssertTrue(try XCTUnwrap(subject.customHeaders?.contains { $0.headerField == "Accept" && $0.value == "application/vnd.github.squirrel-girl-preview" }))
    }
    ...
    func testHandleOpenURL() throws {
        ...
        let url = try XCTUnwrap(URL(string: "urlscheme://authorize?code=dhfjgh23493&state="))
        var token: String?
