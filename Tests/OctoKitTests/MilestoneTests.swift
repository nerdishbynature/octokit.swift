...
class MilestoneTests: XCTestCase {
    func testGetMilestone() {
        let session = OctoKitURLTestSession(
            expectedURL: "https://api.github.com/repos/octocat/Hello-World/milestones/1",
            expectedHTTPMethod: "GET",
            jsonFile: "milestone",
            statusCode: 200
        )
        
        let task = Octokit(session: session).milestone(
            owner: "octocat",
            repo: "Hello-World",
    ...
    func testGetMilestones() {
        let session = OctoKitURLTestSession(
            expectedURL: "https://api.github.com/repos/octocat/Hello-World/milestones?direction=desc&sort=created&state=open",
            expectedHTTPMethod: "GET",
            jsonFile: "milestones",
            statusCode: 200
        )
        
        let task = Octokit(session: session).milestones(
            owner: "octocat",
            repo: "Hello-World") { response in
                switch response {
                case let .success(milestone):
    ...
    func testUpdateMilestone() {
        ...
        let task = Octokit(session: session).updateMilestone(
            owner: "octocat",
            repo: "Hello-World",
            number: 1) { response in
                switch response {
    ...
    func testDeletMilestone() {
        ...
        let task = Octokit(session: session).deleteMilestone(
            owner: "octocat",
            repo: "Hello-World",
            number: 1) { response in
