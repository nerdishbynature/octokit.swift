...
class GistTests: XCTestCase {
    ...
    func testGetGist() {
        let session = OctoKitURLTestSession(
            expectedURL: "https://api.github.com/gists/aa5a315d61ae9438b18d",
            expectedHTTPMethod: "GET",
            jsonFile: "gist",
            statusCode: 200
        )
    ...
    func testPostGist() {
        let session = OctoKitURLTestSession(
            expectedURL: "https://api.github.com/gists",
            expectedHTTPMethod: "POST",
            jsonFile: "gist",
            statusCode: 200
        )
        let task = Octokit().postGistFile(
            session,
            description: "Test Post",
            filename: "Hello-World.swift",
            fileContent: "Sample Program",
            publicAccess: true
        ) { response in
            switch response {
            case let .success(gist):
    ...
    func testPatchGist() {
        ...
        let task = Octokit().patchGistFile(
            session,
