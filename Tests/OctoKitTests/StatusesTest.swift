...
class StatusesTest: XCTestCase {
    ...
    func testCreateCommitStatus() {
        let session = OctoKitURLTestSession(
            expectedURL: "https://api.github.com/repos/octocat/Hello-World/statuses/6dcb09b5b57875f334f61aebed695e2e4193db5e",
            expectedHTTPMethod: "POST",
            jsonFile: "status",
            statusCode: 201
        )

        let task = Octokit().createCommitStatus(
            session,
            owner: "octocat",
    ...
    func testListCommitStatuses() {
        let session = OctoKitURLTestSession(
            expectedURL: "https://api.github.com/repos/octocat/Hello-World/commits/6dcb09b5b57875f334f61aebed695e2e4193db5e/statuses",
            expectedHTTPMethod: "GET",
            jsonFile: "statuses",
            statusCode: 200
        )

        let task = Octokit().listCommitStatuses(
            session,
            owner: "octocat",
            repository: "Hello-World",
